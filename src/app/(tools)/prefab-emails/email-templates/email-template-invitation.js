...
export default class Prefab {
    ...
    dateToFormat(dateString, formatString = 'EEEE d MMMM yyyy') {
    ...
    get email() {
        return `
           ...
           Jij hebt een aanvraag gedaan voor het volgen van de opleiding <strong>${this.course.value}</strong> bij VDAB. Graag informeer ik of het voor jou mogelijk is om op <strong>${this.dateToFormat(this.date.value, 'EEEE d MMMM')}</strong> te starten met de opleiding?
           ...
           Kan jij mij laten weten of je op <strong>${this.dateToFormat(this.date.value, 'dd-MM-yyyy')}</strong> kan starten? Indien niet, dan bepalen we een andere startdatum. Zodra deze vast staat, volgt er een mail met meer informatie over de opleiding.
           ...
           <ul>
               <li>of je kan starten op <strong>${this.dateToFormat(this.date.value, 'dd-MM-yyyy')}</strong></li>
